...
def post_list(request):
    ...
    return render(request, 'shop/post_list.html', args)

def post_home(request):
    args = {}
    if auth.get_user(request).pk != None:
     args['profile'] = UserProfile.objects.get(user = auth.get_user(request))
    return render(request, 'shop/carousel.html', args)
...
def post_detail(request, pk):
        ...
        if auth.get_user(request).pk != None:
            args['profile'] = UserProfile.objects.get(user = auth.get_user(request))
        args['bets'] = Bet.objects.filter(product = args['post']).order_by('bet')
        return render(request, 'shop/post_detail.html', args)
...
def post_make_bet(request, pk):
        ...
        if request.POST:
                    ...
                    if args['bform'].is_valid() and int(args['bform'].cleaned_data.get("bet")) <= int(balance):
                        ...
                        bet.save()
                        # args['profile'].update()
                        return redirect('shop.views.post_detail', pk=pk, )
        else:
            args['bform'] = BetForm(balance= balance)
        return render(request, 'shop/post_make_bet.html', args)
...
def post_result_and_delete(request, pk):
        ...
        if auth.get_user(request).pk != None:
                args['profile'] = UserProfile.objects.get(user=request.user)#UserProfile.objects.get(user = auth.get_user(request))
        args['post'] = get_object_or_404(Post, pk=pk)
        args['bets'] = Bet.objects.filter(product = args['post'])
        ...
        return render(request, 'shop/post_set_result.html', args)
...
def show_result(request, pk):
    ...
    if auth.get_user(request).pk != None:
                args['profile'] = UserProfile.objects.get(user=request.user)#UserProfile.objects.get(user = auth.get_user(request))
    args['post'] = get_object_or_404(Post, pk=pk)
    args['bets'] = Bet.objects.filter(result = args['post'].result, product = args['post'])
    return render(request, 'shop/post_results.html', args)
...
def post_new(request):
    ...
    if request.method == "POST":
            ...
            if args['form'].is_valid():
                ...
                post.save()
                return redirect('shop.views.post_detail', pk=post.pk, )
    else:
            args['form'] = PostForm()
    return render(request, 'shop/post_edit.html', args)
...
def post_edit(request, pk):
        ...
        if request.method == "POST":
            ...
            if args['form'].is_valid():
                ...
                args['post'].save()
                return redirect('shop.views.post_detail', pk=args['post'].pk)
        else:
